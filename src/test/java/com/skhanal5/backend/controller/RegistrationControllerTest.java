...
public class RegistrationControllerTest {
    ...
    void testRegisterUser() throws Exception {
        ...
        mockMvc
                .perform(post("/api/v1/register").contentType(MediaType.APPLICATION_JSON).content(requestBody))
    ...
    void testLoginUser() throws Exception {
        ...
        when(userRepository.findByEmailAddressAndPassword(any(), any())).thenReturn(Optional.of(foundUser));

        mockMvc
                .perform(post("/api/v1/login").contentType(MediaType.APPLICATION_JSON).content(requestBody))
                .andExpect(status().isOk())
                .andExpect(content().string("Found user: null")); // should return a session/cookie of some sort
    }
